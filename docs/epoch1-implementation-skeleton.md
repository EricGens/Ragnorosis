...
*Companion to the full GDD (ragnorosis\_gdd.md, currently v2.1). This document specifies what to build for
...
passively each Pulse**, computed as `Σ (Popularity\_fraction × Population / 1,000,000)` summed across every
...
**Building bonus, both facilities, same formula:** `output\_per\_point = base × (1 + 0.20 × level)` — **linear**
