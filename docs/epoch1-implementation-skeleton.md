...
the Tech epoch, since it requires Molecular Machinery). **Base costs revised down from their original values
after real playtesting** — a uniform 5× reduction across all five (originally 5,000 / 8,500 / 7,000 / 5,000 /
10,000), suggesting the whole cost scale sat too high relative to Production income rather than any single
building being individually miscosted.

| Building | Base cost (Production) | Function |
|---|---|---|
| **Fossil Fuel Plant** | 1,000 | Energy → Production, see §4.4 |
| **Renewable Plant** | 1,700 | Energy → Production, see §4.4 |
| **Production Facility** | 1,400 | Equipment/construction efficiency, see §4.5 |
| **Training Facility** | 1,000 | Manpower efficiency, see §4.5 |
| **Fortification** | 2,000 | +5 Defensibility per level (§1.4), tentative level-10 cap |
...
**Air/Sea Superiority — this placeholder's planned transition has now actually happened.** Was dev-editable
in Epoch 1 specifically because no real domain-control computation existed yet, needed to simulate blockade
behavior ahead of when that system would exist. **Epoch 2 built real domain control and removed this
override entirely** (Epoch 2 doc §7) — the exception's own justification (no real formula existing) expired
once one did, and the override was retired rather than kept as a parallel path alongside real computation.
**Moved to §5.2 below**, where it now belongs alongside every other computed value. Testing blockade
behavior going forward uses the actual mechanism the game itself now has for it — toggling Faction
hostility or Country war status (Epoch 2 doc §1.6) — rather than a value override that could drift out of
sync with what real combat would actually produce.
...
- **Air/Sea Superiority** — moved here from §5.1 once Epoch 2 built real domain control (its full mechanism:
  Epoch 2 doc §1.1–1.5). No direct-override devtool, matching Production and Research's own resolution
  above — edit the actual inputs (Faction relationships, Country war status, Task Force composition and
  positioning) instead of the computed percentage itself.
